...
public class SistemaProduccion{
  ...
  public boolean encadenamientoAdelante(){
    ...
    while((BaseHechos.getInstance().getHecho(meta) == null) && (!CC.isEmpty())){
      ...
      if(BaseHechos.getInstance().getHecho(meta) == null) CC.addAll(equipararAdelante(CC)); //Todas las nuevas reglas que se agregen al Conocimientos "CC" se agregaran despues de las que ya contiene
      // actualmente!
      else {
        finTabla();
        return true;
  ...
  public ArrayList<Regla> resolverAdelante(ArrayList<Regla> CC){
    ...
    BaseHechos.getInstance().addHecho(reglaParaDisparar.getAccion()); //Agregar el NH a la Base de Hechos
  ...
  private void imprimirCabeceraEncadenamientoAdelante(){
    System.out.println("\nSe empleara el metodo de: Encadenamiento Hacia Adelante");
    ...
    System.out.println(" ------------------------------------------------------------------");
    System.out.println("| Conocimientos     | Base de Hechos      | Regla| NH    | Meta|");
    System.out.println(" ------------------------------------------------------------------");
  }
